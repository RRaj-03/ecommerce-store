...
/**
 * Store register proxy.
 * 1. Forward registration data to admin backend
...
export async function POST(req: Request) {
  try {
    ...
    const backendRes = await fetch(`${adminUrl}/register`, {
      method: "POST",
    ...
    if (!backendRes.ok) {
      return NextResponse.json(data, { status: backendRes.status, headers: corsHeader });
    }
    ...
    console.error("[STORE_REGISTER_PROXY]", error);
